...
const RefDemo = () => {
    ...
    return (
        <Flex.column gap={10} full>
            <Typo.span size="s" color="greys.shade60">
                Common pattern: create a ref, attach it to the scroll container, pass only{" "}
                <Typo.span weight="bold">ref</Typo.span> to the hook (no target).
            </Typo.span>
            <Flex
                ref={ref}
                width={180}
                ...
                radius={6}
            >
                Ref is on this box. Long wrapped content. Long wrapped content. Long wrapped
                content.
            </Flex>
            <OverflowReadout
                isOverflowing={isOverflowing}
...
const TargetWindowDemo = () => {
    const [targetKey, setTargetKey] = useState("window");
    const target = PAGE_TARGETS[targetKey]?.() ?? null;

    const { isOverflowingX, isOverflowingY, isOverflowing } = useCheckOverflow({ target });

    return (
        <Flex.column gap={10} full>
            <Typo.span size="s" color="greys.shade60">
                No ref here. You cannot put a ref on window or document.body — pass them as target.
                The hook uses page scroll size vs viewport (see measureOverflow in the hook).
            </Typo.span>
            <Flex gap={8} wrap>
                <Button
                    label="window"
                    onClick={() => setTargetKey("window")}
                    activeManually={targetKey === "window"}
                />
                <Button
                    label="documentElement"
                    onClick={() => setTargetKey("documentElement")}
                    activeManually={targetKey === "documentElement"}
                />
                <Button
                    label="body"
                    onClick={() => setTargetKey("body")}
...
const Demo = () => (
    <Flex.column gap={12} full>
        <Typo.span size="s" color="greys.shade60">
            Four boxes in one view. Each box uses its own hook instance. Resize the panel to refresh
            checks.
        </Typo.span>
        <Flex gap={12} wrap full>
            ...
            <OverflowCase title="Y only (x false, y true)" width={140} height={56}>
                Very long wrapped content. Very long wrapped content. Very long wrapped content.
                Very long wrapped content. More lines here.
            </OverflowCase>
            ...
            <OverflowCase title="Both true (x true, y true)" width={100} height={48}>
                <Flex.column
                    gap={4}
                    style={{
                        width: 240,
    ...
    >
        <Ds.block
            ...
            code={`import { useCheckOverflow } from "${SYS.basePath}";
                    ...
                    const ref = useRef(null);
                    const { isOverflowingX, isOverflowingY, isOverflowing } =
                        useCheckOverflow({ ref });`}
            example={<Demo />}
        />
        <Ds.block
            title="Using ref"
        ...
        <Ds.block
            ...
            code={`import { useCheckOverflow } from "${SYS.basePath}";
                    ...
                    // Page taller/wider than the viewport
                    useCheckOverflow({ target: window });

                    // Or document.documentElement / document.body
                    useCheckOverflow({ target: document.body });`}
            example={<TargetWindowDemo />}
        />
        <Ds.api
            ...
            props={{
                ...
                target: {
                    description:
                        "Element or Window without a React ref (e.g. window, document.body, parent scrollEl, querySelector). Overrides ref when both are set.",
                    type: "Element | Window",
                },
            }}
            returnProps={{

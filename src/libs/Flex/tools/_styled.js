...
const shouldEmitPadding = (v) => isShellValue(v) && v !== "0" && v !== 0 && `${v}`.trim() !== "0";
    ...
    container: styled.div`
        ...
        ${(p) =>
            p.$order != null
                ...
                : ""}
        ${(p) =>
            isShellValue(p.$width)
                ? css`
                      width: ${p.$width};
                  `
                : ""}
        ${(p) =>
            isShellValue(p.$height)
                ? css`
                      height: ${p.$height};
                  `
                : ""}
        ...
        ${(p) =>
            isShellValue(p.$maxHeight)
                ...
                : ""}
        ${(p) => {
            const locked = p.$shellOverflowLocked;
            const o = p.$overflow;
            const ox = p.$overflowX;
            const oy = p.$overflowY;

            if (o != null && ox == null && oy == null) {
                return css`
                    overflow: ${o};
                `;
            }
        ...
        ${(p) => {
            ...
            return css`
                ${shouldEmitPadding(pt)
                    ? css`
                          padding-top: ${pt};
                      `
                    : ""}
                ${shouldEmitPadding(pr)
                    ? css`
                          padding-right: ${pr};
                      `
                    : ""}
                ${shouldEmitPadding(pb)
                    ? css`
                          padding-bottom: ${pb};
                      `
                    : ""}
                ${shouldEmitPadding(pl)
                    ? css`
                          padding-left: ${pl};
                      `
                    : ""}

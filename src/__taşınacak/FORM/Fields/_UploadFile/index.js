...
export const FieldUploadFile = ({ form, ...rest }) => (
    <FORM.field form={[UploadFile, "uploadFile", form?.[0], form?.[1]]} {...rest} />
);

export const UploadFile = (props = {}) => {
    const { fieldVersion, value, name, inputProps, fieldRef, onChange, onClick } = props;
    const {
        maxCount = 1,
            ...
            >
                {(!value || value.length < 1) && (
                    <div id="addArea">
                        ...
                        <span>Click or drag {maxCount > 1 ? "files" : "file"}</span>
                    </div>
                )}
            </Dragger>
        </Version>
